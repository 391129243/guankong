...
public class CountDownProgressBar extends View {
    ...
    private void drawCircle(Canvas canvas, int center, int radius) {
        ...
        alphaAngle = currentValue * 360.0f / duration * 1.0f; // 计算每次画圆弧时扫过的角度，这里计算要注意分母要转为float类型，否则alphaAngle永远为0
    ...
    private void drawText(Canvas canvas, int center) {
        int result = (duration - currentValue); // 计算进度
        String percent;
        if (duration == currentValue) {
            percent = (String) getResources().getText(R.string.timeout_text);
        ...
        if (animator != null) {
            ...
            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    //一般只是希望在View发生改变时对UI进行重绘。invalidate()方法系统会自动调用 View的onDraw()方法。
                    invalidate();
                    if (duration == currentValue && CountDownProgressBar.this.listener != null) {
